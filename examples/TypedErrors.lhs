...
\href{https://www.parsonsmatt.org/2018/11/03/trouble_with_typed_errors.html}
{The Trouble with Typed Errors} that talks about the difficulties we face in Haskell
...
in short:
\begin{itemize}
\item As is, |foo| and |bar| aren't in the same monad (because they have different
  error types!), so we cannot use do notation.
\item If we group the errors into something like |Either FooErr BarErr|, then not
...
\item If we combine the errors into one monolithic error type, we lose static
